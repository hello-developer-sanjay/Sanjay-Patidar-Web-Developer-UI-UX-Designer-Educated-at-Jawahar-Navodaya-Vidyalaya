...
const ProtectedPage = () => {
  ...
  const [userDetails, setUserDetails] = useState([]);
    ...
    useEffect(() => {
      ...
      const fetchUserDetails = async () => {
        try {
          const userDetailsResponse = await axios.get('https://portfolio-forblogs-zdl0.onrender.com/api/userdetails');
  ...
  useEffect(() => {
    ...
    const fetchUserVisitedLocations = async () => {
      try {
        const userVisitedLocationsResponse = await axios.get('https://portfolio-forblogs-zdl0.onrender.com/api/uservisiteds');
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          ...
          const queriesResponse = await axios.get('https://portfolio-forblogs-zdl0.onrender.com/api/queries');
          setQueries(queriesResponse.data.map(query => ({ ...query, resolved: false })));
  
          // Fetch user profiles after successful authentication
          const userProfilesResponse = await axios.get('https://portfolio-forblogs-zdl0.onrender.com/api/userprofiles');
    ...
    const handlePasswordSubmit = async () => {
      try {
        const response = await axios.post('https://portfolio-forblogs-zdl0.onrender.com/api/authenticate', { password });
        if (response.data.authenticated) {
          setAuthenticated(true);
  
          // Fetch feedbacks after successful authentication
          const feedbacksResponse = await axios.get('https://portfolio-forblogs-zdl0.onrender.com/api/feedbacks');
